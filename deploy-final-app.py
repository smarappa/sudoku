...
bucket_name = f'sudoku-final-{int(time.time())}'

print(f"Creating S3 bucket: {bucket_name}")
s3.create_bucket(Bucket=bucket_name)

# Create CloudFront OAI first
...
canonical_user_id = oai_response['CloudFrontOriginAccessIdentity']['CloudFrontOriginAccessIdentityConfig']['CallerReference']

# Grant CloudFront access using canonical user ID
bucket_policy = {
    "Version": "2012-10-17",
    "Statement": [{
        "Effect": "Allow",
...
# Now set bucket policy
try:
    s3.put_bucket_policy(Bucket=bucket_name, Policy=json.dumps(bucket_policy))
    print("Bucket policy applied")
except Exception as e:
    print(f"Bucket policy warning: {e}")
...
print(f"\nAccess Methods:")
print(f"1. CloudFront Distribution: https://{domain_name}")
print(f"2. Custom Domain (when configured): https://samplesrini.com")
print(f"\nHosted Zone Created: /hostedzone/Z00893923ERUMJ5TFJ02U")
print(f"Configure these name servers with samplesrini.com registrar:")
print(f"   - ns-794.awsdns-35.net")
print(f"   - ns-1684.awsdns-18.co.uk") 
print(f"   - ns-79.awsdns-09.com")
print(f"   - ns-1366.awsdns-42.org")